...
def test_materialize_route_candidate_edges_writes_reference_edges(tmp_path) -> None:
    ...
    assert len(edges) == 2
    edge = next(item for item in edges if item.source_record_id == "public-route-ttf-bbl-nbp")
    assert edge.from_node_id == "entsog-vtp-00002"
    assert edge.to_node_id == "entsog-vtp-00006"
