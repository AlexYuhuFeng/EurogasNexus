...
def test_development_profile_includes_dev_route_only() -> None:
    ...
    dev_response = dev_client.get("/dev/health")

    assert dev_response.status_code == 200
    assert dev_response.json() == {"status": "ok", "scope": "development"}
    assert dev_client.get("/internal/health").status_code == 404
...
def test_internal_profile_includes_internal_routes_only() -> None:
    ...
    assert internal_client.get("/internal/health").status_code == 200
    assert internal_client.get("/dev/health").status_code == 404
