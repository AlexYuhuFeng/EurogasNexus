...
def test_cleanup_verifier_covers_backend_and_frontend_payload_surfaces() -> None:
    ...
    assert "Terminology cleanup verified." in text
...
def test_backend_route_response_no_longer_contains_legacy_field() -> None:
    assert LEGACY_FIELD not in ROUTE_COST.read_text(encoding="utf-8-sig")
