...
def test_source_center_and_topbar_are_extracted_from_app() -> None:
    ...
    assert 'from "@/components/SourceCenter"' in app
    assert 'from "@/components/WorkspaceTopBar"' in app
    assert "Data Source Center" not in app
    assert "source-category-list" not in app
...
def test_network_cockpit_exposes_warning_evidence_stack() -> None:
    ...
    assert "reviewEvidenceItems" in app
