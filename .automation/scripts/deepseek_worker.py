...
def main():
    ...
    env=os.environ.copy();env['CODEX_HOME']=str(home);flags=subprocess.CREATE_NEW_PROCESS_GROUP if os.name=='nt' else 0;proc=subprocess.Popen(cmd,cwd=root,env=env,stdin=subprocess.DEVNULL,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,creationflags=flags,start_new_session=(os.name!='nt'))
