...
| `AllInOne` | 演示、试用或单用户工作站 | 完整 Server 加桌面客户端 | 同一设备上的 HTTPS 地址 | 仅后端服务 |
...
- 中英文运维说明；
- 作为独立 Release 产物提供的签名 NSIS 客户端安装包。
...
AllInOne 是在同一设备安装 Server 和 Client。它需要与 Server 相同的证书和
Docker 条件，并额外需要 NSIS 安装包：
...
模拟价格必须通过 `-EnableSimulatedPrices` 显式启用。模拟源与正式授权源走完全
相同的采集、标准化、PostgreSQL、API、SDK 和客户端链路，并始终保留 `_Sim`
来源标记。未来替换为正式连接器时，不改变客户端工作流和存储边界。
