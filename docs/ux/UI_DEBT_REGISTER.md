...
contains evidence from multiple refs; the current closure context is HEAD
`9a84f5e`. Functional closure does not close separate layout, visual, or
acceptance debt.
...
| ID | Screen / workflow | Severity | Description | Reason deferred | Recommended solution |
| --- | --- | --- | --- | --- | --- |
| UX01-001 | Global shell, Network and all workspaces | P1 confirmed | Topbar, navigation, context/status controls, map controls, and rail tabs share a band; wrapped controls overlap analytical content. | Confirmed in the audit captures; implementation is pending, while 1920x1080, narrow desktop, and all-workspace coverage remain open. | Implement structural shell rows with measured height; content starts below them; verify normal, loading, degraded, restricted, and empty states. |
| UX01-002 | Network offline/degraded error state | P1 confirmed | Raw endpoint-name output spans the top edge and collides with alerts/navigation; retry and error treatment are not bounded. | Confirmed offline evidence; normal and endpoint-specific error coverage is still incomplete. | Use one bounded, structured error surface with useful summary, safe details, retry behavior, and normal layout participation. |
| UX01-SCENARIO-001 | Portfolio -> Optimize/Review -> Scenario | P1 candidate - confirm current scope | Closed handoff navigation is separate. Unresolved source risk: `usePortfolioDecisionModel.ts:120` assigns `selectedAllocation` from `api.routeRecommendation?.allocations[0]`; lines 135, 138, and 142 use it as fallback for decision margin, sale price, and route charge. Portfolio route rows match allocations by `route_id` at `PortfolioWorkspace.tsx:114-116`, but the Scenario-facing economics are not visibly keyed to a selected route in this model. A first-allocation result could therefore describe another route; runtime confirmation is pending. | The corrected handoff passed navigation evidence; current selected-route economics require a runtime trace with multiple allocations and exact endpoint/result identity before severity is assigned. | Trace `selectedRoute`/`route_id` through the model and Scenario surface; use the matched allocation, show Unknown/Missing when no match exists, and record operation/fixture/result evidence. Preserve the closed handoff contract. |
...
| ID | Screen / workflow | Severity | Description | Reason deferred | Recommended solution |
| --- | --- | --- | --- | --- | --- |
| EVID-DESKTOP-001 | Native WebView/Tauri desktop | Evidence gap | A native build at clean `9a84f5e` exists: 9,142,272 bytes with parent-reverified SHA-256 `124A148C6954F7536159F7DFBE413216C7E83F4A37EA203FBF831CCA6EDF41BC`. The artifact establishes build identity, not native interaction acceptance. | The older executable was correctly rejected as stale; rebuilding is not the next action. Native launch, interaction, accessibility, and full page walkthrough against the 9a84f5e build remain pending. | Inspect the existing 9a84f5e artifact and record native screenshots, accessibility tree, navigation, stale/degraded states, and package identity; do not infer behavior from compilation. |
| EVID-SUITE-001 | Whole-product Web and desktop acceptance | Evidence gap | Pending coverage includes browser/visual 1440x900, 1920x1080 and narrow desktop, axe/accessibility, EN/CN, keyboard/focus, long-session, populated research/agent workflows, rights negatives, and current-ref desktop checks. | The census is source evidence; targeted historical tests/builds and partial captures are not a current whole-product pass. | Parent acceptance run records exact commands, SHA, environment, fixture/operation IDs, screenshots, skips, and failures. Do not convert missing evidence into P1 or PASS. |
...
- This register contains no source edits, fixture definitions, test results,
  visual pass claims, or P2 acceptance decisions.
