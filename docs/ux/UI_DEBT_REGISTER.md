...
| ID | Screen / workflow | Severity | Description | Reason deferred | Recommended solution |
| --- | --- | --- | --- | --- | --- |
| CR14-RIGHTS-001 | Dataset build/detail/export | P1, scoped correction implemented | API and agent build now pass authenticated scopes into pre-query canonical source filtering. Runtime row/snapshot export policy is derived server-side; list/detail/quality/agent reads require persisted trusted source IDs and current grants. Legacy/unknown provenance and forged export envelopes fail closed. | Parent verified 78 scoped tests, not full PostgreSQL or end-to-end entitlement acceptance. Canonical registry has no proven export clearance; actual export artifact workflow is still absent. | Complete live PostgreSQL/identity-negative coverage, reviewed legacy provenance handling, and artifact/export adapter acceptance before closing the full requirement. |
