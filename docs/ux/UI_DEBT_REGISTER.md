...
| ID | Screen / workflow | Severity | Description | Reason deferred | Recommended solution |
| --- | --- | --- | --- | --- | --- |
| UX01-001 | Global shell, Network and all workspaces | P1 confirmed | Topbar, navigation, context/status controls, map controls, and rail tabs share a band; wrapped controls overlap analytical content. A candidate normal-flow grid implementation exists in `clients/web/src/styles/app.css`; a reviewed screenshot shows context uncut, but only in loading/unavailable state. | Confirmed audit debt remains open; the candidate CSS is uncommitted and has no populated-data screenshot or full viewport/state acceptance. | Review populated normal, degraded, restricted, and empty states at required viewports; accept only after screenshots show measured shell rows with content below them and no overlap. |
...
| UX01-SCENARIO-001 | Portfolio -> Optimize/Review -> Scenario | Closed functional - wrong-route association only; browser evidence open | Parent visually reviewed `output/playwright/ux01-scenario-second-route-1440.png` for `public-route-ttf-local`: source resource-pool allocation sale `27.05`, resource volume `8000`, cost `n/a`; Kant-matched response `gross_sale=27.0472`, `totalcost=25.0219`, `allocated=8000`, with no recommendation response. The candidate helper matches carried `route_id` and explicit pool `option_id`/resource fallback. | Closure is limited to the source association defect. The `routeRecommendation` browser fixture is absent; focused unit coverage is `73 passed` including explicit default and ambiguous-pool cases, and parent reports `71/71` Web tests. No full-suite, degraded-state, or broader Scenario/layout acceptance is claimed. | Preserve the helper and closed navigation contract. Add the browser fixture later and verify selected-route label/economics against the exact response; do not reopen this specific functional closure for unrelated layout debt. |
