...
| CR-11 | Reliability, health/readiness, metrics, recovery, migration, backup/restore, and performance operations. | `src/eurogas_nexus/application/` reliability/dataops modules, health/runtime routes, `scripts/ops/migration_preflight.py`, `backup_restore_drill.py`, `performance_baseline.py`, `load_smoke.py`, `recover_stale_jobs.py`, `docs/operations/PRODUCTION_RELIABILITY_SPEC.md`, and `BACKUP_RESTORE.md`. | `tests/unit/test_reliability_ops.py`, `tests/contract/test_reliability_migration.py`, `tests/integration/test_migration_failure.py`, `tests/integration/test_db_health.py`; CI runs `load_smoke.py` and the PostgreSQL job. **Current-ref evidence added 2026-09-18** (local PostgreSQL 16, `eurogas_nexus`): `alembic upgrade head` applied `0033`→`0036` in one transaction, `validate_runtime_db.py` reports every required table present, the automated restore drill passed (52.5 MB dump restored into an isolated database at head, 80,545 ingestion runs, API smoke 200), and the performance baseline measured p50 39.6 ms / p95 1295 ms / p99 2313 ms against the 1500 ms / 2500 ms targets. | Implemented | Migration, restore and performance now verified on the current ref; recovery of stale jobs and the *production* drill still unrun | CI does not run every operational command in `docs/operations/VALIDATION.md`, and the container/deployment target differs from this local one, so the evidence above is current-ref but not production. |
| CR-12 | Release/update/install surfaces, compatibility blocking, packaging, signing, SBOM, provenance, and release gates. | `clients/web/src/app/releaseCompatibility.ts`, `AppShell.tsx`, `SettingsCenter.tsx`, `/api/runtime/release`, `scripts/release/`, release workflow, and `docs/release/RELEASE_ENGINEERING_SPEC.md`/`UPDATE_POLICY.md`. | `clients/web/tests/releaseCompatibility.test.ts`; `tests/release/test_release_engineering.py`, `test_release_readiness_contract.py`; CI/release workflow includes packaging and release validation. **Local dry run 2026-09-18** (`run_release_dry_run.py --channel preview --build-container`): web package, deployment bundle, container image (`sha256:f6f5d6bf…`), SBOM (SPDX 2.3), signing state (unsigned/pending-external), vulnerability scan (PASS) and checksums/manifest all OK; `package-windows` needs the desktop bundle and the gate reports `G18` user acceptance as `PENDING_EXTERNAL`. | Implemented compatibility/release surfaces; updater intentionally not shipped | Locally runnable release steps verified; external gates pending | Compatibility and release evidence machinery exist, but `docs/release/UPDATE_POLICY.md` explicitly says no Tauri updater ships. Clean install/launch/uninstall, previous-version upgrade, Authenticode signing, hosted provenance, Linux packaging matrix, and stable external gates remain pending. These are release prerequisites, not automatically P1 UX defects. |
| CR-13 | Commercial UAT, accessibility, i18n, final workflows, degraded states, long session, and acceptance evidence. | UAT plans/reports/defect register, `scripts/uat/check_i18n_parity.py`, `seed_uat_fixture.py`, `browser_workflow_smoke.mjs`, Web i18n files, and `tests/uat/`/`tests/evals/` are present. | Historical `docs/uat/RC_ACCEPTANCE_REPORT.md` reports Golden A-F, 0 axe violations across 13 URLs, 1,247/1,247 key parity, and long-session smoke. **This row was wrong about CI**: `.github/workflows/ci.yml` has a `browser-acceptance` job that seeds a migrated PostgreSQL UAT runtime, starts the API and Vite, and runs the Playwright/axe sweep across 16 workspaces × EN/zh-CN × 3 viewports. CI run **35356450358** at `0c34959` (and run 448 at `efcbddb`) passed it: 96/96 checks, 0 axe violations, one h1 per surface, no overflow. The sweep was also run locally on 2026-09-18 against the upgraded map library; `scripts/uat/check_i18n_parity.py` reports 2,399 keys per locale with no divergent key. | UAT artefacts implemented; browser/axe/i18n now verified on the current ref | Verified on the current ref; the AT/long-session halves remain unrun | The browser script needs separately installed Playwright and seeded services (CI installs both), and no Playwright or axe dependency is in `clients/web/package.json` by design. Assistive-technology user testing, long-session behaviour and live/licensed-data acceptance are still open, and none of them is closed by the matrix being green. |
...
| CR-15 | Capability Registry/MCP, governed agent research orchestration, ResearchPlan, StrategyIR, Risk Challenger, replay, entitlements, and human gates. | `src/eurogas_nexus/application/agents/`, `domain/agents/`, agent models/repositories, `api/routes/public/agents.py`, `mcp/server.py`, `docs/agents/`, and `AgentsWorkspace.tsx` are present. | `tests/unit/test_agent_capability_runtime.py`, `test_mcp_capability_adapter.py`, `test_mcp_server.py`; `tests/domain/agents/test_agent_contracts.py`; `tests/api/test_agents_api.py`; `tests/integration/test_agent_repository.py`, `test_agent_orchestrator.py`; `tests/evals/test_agent_evals.py`; CI has an MCP handshake smoke. **The rendered chain is now evidenced**: the browser sweep's agent-research interaction drives a run from the research workspace to `READY_FOR_HUMAN_REVIEW` and asserts the replay's artefact chain is complete (`research_plan`, `findings`, `strategy_ir`, `validation`, `challenge_report`, `review_pack`), then confirms the review pack through the review API and re-reads the run. | Backend implemented; the rendered plan → findings → StrategyIR → validation → challenge → review-pack flow is now verified end to end | Verified on the current ref; live LLM grading remains external | Backend contracts cover permission/entitlement/no-execution/human-confirmation paths, and the rendered chain is asserted by the sweep rather than inferred. Live LLM grading is explicitly external/pending; robustness beyond the CR-04 engine is deferred. |
