...
- `python -m pytest tests -q --ignore=tests/integration` — **1655 passed, 1 skipped, 0 failed**.
- `clients/web`: `node --test "tests/*.test.ts"` — **352 passed**; `npx tsc --noEmit` exit 0; `npm run build` exit 0.
...
- **Wave 9** — the panel taxonomy is still declared but unapplied; the action geography has its first application (the Optimize task's run sits in the workspace primary slot) and is now enforced by a surface-scanning suite, but the other surfaces still keep their primary actions inside panels; six surfaces hand subjects to the Inspector (the market cockpit, the contract workbench, the network map, the strategy backtest, the review decision history and the capacity point panel); the shell composes every page through the workspace renderer and intercepts none of them, and the legacy `orders` deep link opens the view it names, so conflicting-register entries C9 and C10 are closed; C11 was re-measured (no component is unmounted, but 49 of 131 client API methods have no call site - the strategy lifecycle, shadow-runtime and access families) and C12 (map-tile licence and client-held token) stays deferred.
