...
- `python -m pytest tests -q --ignore=tests/integration` — **1656 passed, 1 skipped, 0 failed**.
...
- **Wave 7** — every surface now reaches the Copilot through the canonical contract (the palette, the market cockpit and the review task); the Copilot output is not yet attached as Decision Case evidence; all five actions deliberately run the same backend task kind; the `/agent/*` research surfaces are unconverged.
