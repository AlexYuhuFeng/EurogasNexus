...
- **Security finding C8, direct-LLM half and MCP default** — every direct provider invocation is re-authorised against the caller's own `analysis.query` capability (403 `ai_authority_not_granted`), the alert-analysis path is policy-gated rather than READ, the analysis provider path fails closed with an audit record, the alert run is attributed to the identity it ran under, and MCP's pseudo-principal no longer defaults to a wildcard data scope. The agent documentation now states the limits it used to overstate: MCP runs as a deployment-configured service identity, not the calling user, and the legacy MCP read/sandbox tools bypass the capability runtime.

## Validation evidence (combined tree)

- `python -m pytest tests -q --ignore=tests/integration` — **1652 passed, 1 skipped, 0 failed**.
- `clients/web`: `node --test "tests/*.test.ts"` — **344 passed**; `npx tsc --noEmit` exit 0; `npm run build` exit 0.
...
- **Wave 7** — no per-workspace inline Copilot mount and no shell-supplied context yet; the Copilot output is not yet attached as Decision Case evidence; all five actions deliberately run the same backend task kind; the `/agent/*` research surfaces are unconverged.
- **Wave 8** — portfolio reports and governed agent research runs are now tracked with honest artefact references; ingestion runs are still untracked (a worker-side change rather than a request-path one).
- **Wave 9** — the panel taxonomy and the action geography are declared but not applied across every workspace; five surfaces hand subjects to the Inspector (the market cockpit, the contract workbench, the network map, the strategy backtest and the review decision history), the remaining surfaces still show detail locally; the `system`-area views are still not URL-addressable; conflict register entries C9 and C10 stay open.
...
- **Security** — C5 (development/internal profiles install no app-wide authentication dependency and the compatibility principal is unrestricted) stays open and deliberate; C6b's self-service half is closed (a principal cannot change its own roles or data scopes; a second-approver policy for grants remains the ADR question); C7 is **decided** (the route-level `require_entitlement` stays unwired because the per-row, per-family filter is the correct control; the decision travels in the dependency's docstring and its fail-closed behaviour is now tested); the C8 remainder is MCP's inability to inherit the calling user across its transport and the legacy MCP read/sandbox tools that bypass the capability runtime.
