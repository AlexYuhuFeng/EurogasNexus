...
- `python -m pytest tests -q --ignore=tests/integration` — **1647 passed, 1 skipped, 0 failed**.
- `clients/web`: `node --test "tests/*.test.ts"` — **342 passed**; `npx tsc --noEmit` exit 0; `npm run build` exit 0.
...
- **Wave 9** — the panel taxonomy and the action geography are declared but not applied across every workspace; the market cockpit, the contract workbench, the network map and the strategy backtest hand subjects to the Inspector, the remaining surfaces still show detail locally; the `system`-area views are still not URL-addressable; conflict register entries C9 and C10 stay open.
