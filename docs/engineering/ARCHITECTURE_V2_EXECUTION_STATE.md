...
- `clients/web`: `node --test "tests/*.test.ts"` — **340 passed**; `npx tsc --noEmit` exit 0; `npm run build` exit 0.
...
- **Wave 9** — the panel taxonomy and the action geography are declared but not applied across every workspace; the market cockpit and the contract workbench hand subjects to the Inspector, the remaining surfaces still show detail locally; the `system`-area views are still not URL-addressable; conflict register entries C9 and C10 stay open.
