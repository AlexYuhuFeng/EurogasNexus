...
- `clients/web`: `node --test "tests/*.test.ts"` — **347 passed**; `npx tsc --noEmit` exit 0; `npm run build` exit 0.
...
- **Wave 7** — the palette and the market cockpit both reach the Copilot; the review surface's own analysis panel still carries an ad-hoc question box beside the canonical actions; the Copilot output is not yet attached as Decision Case evidence; all five actions deliberately run the same backend task kind; the `/agent/*` research surfaces are unconverged.
- **Wave 8** — every run family V2 names is tracked; the remaining operations questions are job replay and retention policy.
- **Wave 9** — the panel taxonomy and the action geography are declared but not applied across every workspace; six surfaces hand subjects to the Inspector (the market cockpit, the contract workbench, the network map, the strategy backtest, the review decision history and the capacity point panel), the remaining surfaces still show detail locally; the shell composes every page through the workspace renderer and intercepts none of them, and the legacy `orders` deep link opens the view it names, so conflicting-register entries C9 and C10 are closed; C11 (unmounted legacy surfaces) and C12 (map-tile licence and client-held token) stay open.
