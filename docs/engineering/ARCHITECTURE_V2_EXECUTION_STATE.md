...
- [x] **Wave 8** — product error taxonomy with the additive API error envelope; the unified Job model (`0036_job_records`) adopted by four run paths: the dataset build, the resource-pool optimisation, the strategy backtest and - after its best-effort persistence was made honest - the portfolio report, which cites its report only when the store really holds it (`W8-01`, `W8-02`).
- [x] **Wave 9, first and second slices** — the canonical Inspector region and the mounted command palette, then real Inspector detail: `app/model/inspectorDetail.ts` resolves facts and provenance from data the identity already received, presents rather than computes, treats absence honestly, and refuses a hand-over the page's composition does not declare. Every hand-over goes through one composition-checked builder (`inspectorSubjectFor`). Three workspaces are migrated: the market cockpit hands over its focused observation, the contract workbench hands over a saved contract, and the network map hands a clicked node over from its popup (`W9-01`).
...
- `python -m pytest tests -q --ignore=tests/integration` — **1641 passed, 1 skipped, 0 failed**.
- `clients/web`: `node --test "tests/*.test.ts"` — **341 passed**; `npx tsc --noEmit` exit 0; `npm run build` exit 0.
...
- **Wave 8** — portfolio reports are now tracked with an honest artefact reference; agent-research runs and ingestion runs are still untracked; unhandled exceptions still use the framework default rather than a system-fault envelope.
- **Wave 9** — the panel taxonomy and the action geography are declared but not applied across every workspace; the market cockpit, the contract workbench and the network map hand subjects to the Inspector, the remaining surfaces still show detail locally; the `system`-area views are still not URL-addressable; conflict register entries C9 and C10 stay open.
