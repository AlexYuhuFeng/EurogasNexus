...
- `python -m pytest tests -q --ignore=tests/integration` — **1642 passed, 1 skipped, 0 failed**.
...
- **Wave 8** — portfolio reports and governed agent research runs are now tracked with honest artefact references; ingestion runs are still untracked (a worker-side change rather than a request-path one); unhandled exceptions still use the framework default rather than a system-fault envelope.
