...
conflict table above. **Current state, re-measured after slice D:** 132 methods and **no method
without a mention** — every declared client call now has a surface — with 30 still never invoked by
name (they are passed into the loader seam, which is a surface using them). The rows below state each
slice's own movement.
...
| D: reference and data reads | `facilities`, `marketHubs`, `capacityContracts`, `routeCost`; `portfolioLiveSummary` **retired from the client as superseded** | Market — the **reference register** (facilities and market hubs, with the type and country filters the route itself accepts) on the network task beside the map it is the register for; Capacity — the **declared capacity profile book** (`GET /api/contracts/capacity`) on the capacity task; Portfolio — the **route-cost and indicative-netback what-if** in the Routes task, whose `compute` occupies that task's primary slot per the action geography. `api.portfolioLiveSummary` is deleted rather than left uncalled: the Wave 5 projection's `summary` slice is the same `summarize_portfolio` result, narrowed by the caller's entitlement (which the route does not do) and on one as-of with the orders it summarises, so a second client read would answer the same question from a different instant. The route stays for the SDK and the CLI, and the reason is recorded where the projection is declared | **Delivered**: never-mentioned set 5 → 0 |
...
**DECIDED 2026-09-19 — the owner retired the unmounted terminal and kept its content.** The fourth
question of this decision was `StrategyShadowRunTerminal.tsx` (838 lines) and its five section
components: nothing rendered them (two tests read their text), while the Shadow task is served by
`StrategyShadowShell.tsx`, which manages monitors, alerts, drift, evaluations with their risk checks,
and lifecycle. The terminal was not a duplicate - it carried the price-basis board, the market tape,
the weighted pool cost, the daily PnL at risk per basis, contract-level attribution, the allocation
ladder, paper state and run provenance, none of which any mounted surface showed - but it could not
simply be mounted: the controller no longer supplies the fifteen props it declares (there is no
`strategyEvaluate`, `strategyScenario`, `strategyResult`, `strategySummary` or `onReviewRun` today),
and it renders its own `WorkspaceTabs` inside a task that already owns that task's views.

The owner chose to **keep the content and retire the parallel shell**, which is what happened:
the derivations moved to `app/model/shadowRunPresentation.ts` as pure functions over the reads the
client already holds (the price tape, FX, the pool's resources, the strategy runs and the last
evaluated result), the panels render from `StrategyShadowRunDetail.tsx` inside the shadow task beside
the monitor management, and the terminal's tab set, risk-override form and dead props are gone. Two
of its four views are now redundant by construction (the task owns Monitor, and Run History is the
Performance panel), so the spec's "four task-led views" claim went with them and
`docs/clients/WEB_CLIENT_DESIGN_SPEC.md` was updated rather than left describing a component that no
longer exists.

Two things this decision records rather than fixes:

- **Bar size has no operator control.** The terminal's evaluate form offered `bar_minutes` of 1, 5 or
  15; the scenario builder declares 5 for every request, and no mounted surface can change it. The
  contract test that asserted the selector (`test_strategy_bar_minutes_is_operator_selectable`) was
  asserting a control in a component nobody could reach - it now asserts what the product does and
  states the gap, because "operator-selectable" was never true. If a bar size should be selectable,
  the Design task is where it belongs, and it is a scenario input rather than a presentation choice.
- **The retired surfaces' vocabulary remains in the locale files.** Roughly forty keys that only the
  terminal used (`strategy.shadow_terminal`, `strategy.view.*`, `strategy.risk_controls`, …) are now
  unreferenced, as are about a hundred older ones in other families; the analysis that found them
  (literal references plus the template bases that build keys dynamically) is recorded in `W0-04`
  finding 13 rather than acted on, because a locale file is a vocabulary rather than a claim and a
  mass deletion needs that same dynamic-key audit applied to every family.
