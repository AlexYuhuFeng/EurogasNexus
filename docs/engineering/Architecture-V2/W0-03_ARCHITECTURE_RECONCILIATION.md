...
| C11 | 26 shared-client methods have no call-site reference, and one legacy terminal plus its transitively referenced sections are unmounted ([W0-01](W0-01_CLIENT_INVENTORY.md) sections 2.8, 4.7) | V2 04 rule: a capability does not earn a page; dead surfaces are maintenance risk | Wave 7/9 decision with owner confirmation. Reachability, not static reference, is the evidence | **RE-MEASURED — the number depends on what counts as a call site, so both are stated.** A reachability scan finds **no unmounted component**: every `.tsx` under `src` is referenced by source or by a test, so the unmounted-terminal half is closed. For the client methods, scanning the `api` object literal in `clients/web/src/api/client.ts` (two-space-indented `name: (` / `name: async (` entries, measured 2026-02 in the current tree) finds **131 methods**, of which **21 are never mentioned anywhere** in `clients/web/src` outside that file, and **52 are never called directly** (`name(`) — the difference is names passed as references, e.g. `loadWorkspaceEndpoint(api.me, …)`, which are real uses. The decision-relevant set is the smaller one, because a mention through the loader seam is a surface using the method: access administration (`accessDataScopes`, `accessRoles`, `createAccessApiKey`), agents and capabilities (`agentProfiles`, `agentRun`, `invokeCapability`, `searchCapabilities`), the strategy lifecycle (`backtestExperiment(s)`, `createBacktestExperiment`, `strategyRegistryRun`, `updateStrategyMetadata`), the shadow runtime (`shadowEvaluation`, `shadowMonitor`), reference and data reads (`capacityContracts`, `facilities`, `marketHubs`, `routeCost`), `researchCapabilities`, `portfolioLiveSummary` and `recordDecisionCaseDecisionOutcome`. Each is a declared backend capability whose surface does not exist yet, so the decision is *not* a deletion: it is whether to build those surfaces or to retire the routes, and it needs the owner's call. The earlier "49 of 131" figure was a direct-call count and is superseded by the pair above. **OPEN — Wave 7/9 with a re-measured scope** |
