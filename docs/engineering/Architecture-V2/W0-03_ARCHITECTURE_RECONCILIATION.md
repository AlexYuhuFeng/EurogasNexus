...
| C11 | 26 shared-client methods have no call-site reference, and one legacy terminal plus its transitively referenced sections are unmounted ([W0-01](W0-01_CLIENT_INVENTORY.md) sections 2.8, 4.7) | V2 04 rule: a capability does not earn a page; dead surfaces are maintenance risk | Wave 7/9 decision with owner confirmation. Reachability, not static reference, is the evidence | **RE-MEASURED — partly stale.** A reachability scan of the current tree finds **no unmounted component**: every `.tsx` under `src` is referenced by source or by a test, so the unmounted-terminal half is closed (either mounted since, or the inventory's list was narrower than it read). The client-method half has **grown**: 49 of the 131 methods in `api/client.ts` have no call site outside the client itself, and they cluster in three families - the strategy lifecycle (`strategies`, `createStrategyVersion`, `freezeStrategyVersion`, `forkStrategyVersion`, `updateStrategyVersionDraft`, `retireShadowMonitor`, …), the shadow runtime (`shadowMonitors`, `shadowEvaluations`, `shadowDrift`, `pauseShadowMonitor`, …) and access administration (`accessRoles`, `accessDataScopes`, `createAccessApiKey`). Each is a declared backend capability whose surface does not exist yet, so the decision is *not* a deletion: it is whether to build those surfaces or to retire the routes, and it needs the owner's call. **OPEN — Wave 7/9 with a re-measured scope** |
